...
onMounted(async () => {
  ...
  try {
    ...
    const [catsRes, articlesRes] = await Promise.all([
      articleCategoryListService() as any,
    ...
    const result: { name: string; count: number; lastUpdated: string }[] = []
    ...
    /* Fade in UI */
    setTimeout(() => { showUI.value = true }, 300)
  } catch (e) {
...
/* --- Watch selected category from 3D click --- update sidebar + labels --- */
      ...
      <div v-if="showUI" class="vr-overlay">
        ...
        <header class="vr-topbar">
          ...
          <div class="vr-topbar__center">
            ...
            <p class="vr-topbar__subtitle">MY BLOG IN SPACE</p>
          </div>
          ...
          <div class="vr-console__stats">
            ...
            <div class="vr-console__stat">
              <div>
                ...
                <span class="vr-console__stat-label">文章</span>
              </div>
            </div>
          </div>
...
.vr-topbar {
  ...
  padding: 16px 24px;
  pointer-events: auto;
  background: linear-gradient(180deg, rgba(5, 8, 22, 0.6) 0%, transparent 100%);
}
...
.vr-console {
  ...
  justify-content: space-between;
  pointer-events: none;
  background: linear-gradient(0deg, rgba(5, 8, 22, 0.65) 0%, transparent 100%);
}
...
.card-slide-leave-active { transition: all 0.2s ease-in; }
.card-slide-enter-from { opacity: 0; transform: translateX(-50%) translateY(12px) scale(0.95); }
.card-slide-leave-to { opacity: 0; transform: translateX(-50%) translateY(8px) scale(0.97); }
