...
export function svgAnime(base: string, matrices: string[][], duree: number): string {
  ...
  const clip = base.match(/<g[^>]*clip-path="[^"]+"[^>]*>[\s\S]*?<\/g>/)
  if (!clip) throw new Error('yeux introuvables')

  let n = 0
  const clipAnime = clip[0].replace(/\stransform="[^"]*"/g, () => ` class="oeil${n++}"`)
  if (n === 0) throw new Error('aucun oeil a animer')

  const parImage = matrices[0]!.length
  if (parImage !== n) throw new Error(`${n} yeux dans le clip, ${parImage} par image cle`)
  ...
  return base.replace(clip[0], clipAnime).replace('</svg>', `${style}</svg>`)
}
